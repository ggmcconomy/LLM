...
try:
    ...
    st.error(f"Missing secret: {e}. Falling back to environment variables.")
...
# Ensure redirect URI keeps trailing slash if present
...
if not all([MURAL_CLIENT_ID, MURAL_CLIENT_SECRET, MURAL_REDIRECT_URI, MURAL_BOARD_ID]):
    ...
    st.stop()
st.write("DEBUG: Secrets validated")
...
def exchange_code_for_token(code):
    ...
    try:
        st.write("DEBUG: Attempting token exchange")
            ...
            if resp.status_code == 400:
                if "redirect" in error_detail.lower():
                    st.error(
                        ...
                        "https://app.mural.co/developers (e.g., include trailing slash if needed)."
                    )
                else:
                    st.warning(
...
def refresh_access_token(refresh_token):
    ...
    try:
        ...
        st.error(f"Unexpected error during token refresh: {str(e)}")
        return None
...
def pull_mural_stickies(auth_token, mural_id):
    ...
    try:
        ...
        st.error(f"Unexpected error pulling sticky notes: {str(e)}")
        return []
...
if auth_code and not st.session_state.access_token:
    ...
    if tok_data:
        ...
        st.warning("Token exchange failed. Please try reauthorizing.")
